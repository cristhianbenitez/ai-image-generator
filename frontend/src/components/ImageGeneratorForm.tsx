...
type PromptFieldProps = FieldProps & {
  textAreaRef: React.RefObject<HTMLTextAreaElement>;
};

const PromptField = ({ formData, onChange, textAreaRef }: PromptFieldProps) => (
...
const NegativePromptField = ({ formData, onChange }: FieldProps) => (
  <div
  ...
  >
    <label
      id="negative-prompt-label"
      className="text-gray text-label font-semibold"
    >
    ...
    <input
      ...
      onChange={e => onChange({ ...formData, negativePrompt: e.target.value })}
...
const ColorSelector = ({ formData, onChange }: FieldProps) => (
  <div className="form-group" role="group" aria-labelledby="colors-label">
    <label
      id="colors-label"
      className="text-gray text-label font-semibold mb-3"
    >
      Colors
    </label>
    <div
...
const ResolutionSelector = ({ formData, onChange }: FieldProps) => (
...
const GuidanceScaleSlider = ({ formData, onChange }: FieldProps) => (
  <div className="form-group" role="group" aria-labelledby="guidance-label">
    ...
    <input
      ...
      className="w-full accent-purple form-slider"
...
export const ImageGeneratorForm: React.FC<ImageGeneratorFormProps> = ({
  ...
  return (
    ...
    >
      ...
      />
      <NegativePromptField formData={formData} onChange={onChange} />
      <ColorSelector formData={formData} onChange={onChange} />
      <ResolutionSelector formData={formData} onChange={onChange} />
      <GuidanceScaleSlider formData={formData} onChange={onChange} />
      <GenerateButton />
    </form>
  );
};
